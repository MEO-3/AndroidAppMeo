...
class VMAddDevice @Inject constructor() : ViewModel() {
    ...
    fun connectToDevice(device: MDeviceConfigBle) {
        ...
        _ui.value = _ui.value.copy(connecting = true, error = null)
            ...
            try {
                MeoSdk.bleDiscoveryHandler().connectAndIdentifyDevice(device, object: RequestCallback<MDeviceInfo> {
                    override fun onSuccess(var1: MDeviceInfo, var2: String) {
                        viewModelScope.launch(Dispatchers.Main) {
                            _ui.value = _ui.value.copy(deviceId = var1.macAddress ?: "", error = null, connecting = false)
                        }
                    }

                    override fun onFailure(var1: Int, var2: String) {
                        viewModelScope.launch(Dispatchers.Main) {
                            _ui.value = _ui.value.copy(error = var2, connecting = false)
                        }
                    }
                })
            } catch (t: Throwable) {
                ...
                viewModelScope.launch(Dispatchers.Main) {
                    _ui.value = _ui.value.copy(error = t.message ?: "Connect error", connecting = false)
                }
            }
        }
    }
    ...
    fun connectWifi(ssid: String, password: String) {
        _ui.value = _ui.value.copy(provisioning = true, error = null)
        viewModelScope.launch(Dispatchers.IO) {
            try {
